...
<div class="page-content">
   <div class="container-fluid">
      <div class="row">
         <div class="col-12">
            <div class="card mt-2">
               <form action="{{ route('wholesale.update', $invoice->id) }}" method="POST">
					@csrf
					@method('PUT')
                  <div class="card-body row">
                     <div class="col-md-9">
                        <h4 class="card-title">Wholesale Invoice Edit Page</h4>
                        <br><br>
                        <div class="row">
                           <div class="col-md-2">
                              <div class="md-3">
                                 <label for="date" class="form-label">Date</label>
                                 <input class="form-control" name="date" type="date" id="date" value="{{ $invoice->date }}">
                              </div>
                           </div>
                           <div class="col-md-1">
                              <div class="md-3">
                                 <label for="example-text-input" class="form-label">Inv No</label>
                                 <input class="form-control example-date-input" name="invoice_no" type="text" value="{{ $invoice->invoice_no }}" id="invoice_no" readonly style="background-color:#ddd" >
                              </div>
                           </div>
                           ...
                           <div class="col-md-1" style="margin-top:30px;">
                              <div class="md-3">
                                 ...
                                 <i class="btn btn-secondary btn-rounded waves-effect waves-light addeventmore" id="addMoreButton">Add</i>
                              </div>
                           </div>
                        </div>
                        <!-- end row -->
											...
											<tr>
												...
												<td>
													<select name="type[]" class="form-select transaction-type">
														<option value="sale" {{ $detail->type == 'sale' ? 'selected' : '' }}>Sale</option>
														<option value="return" {{ $detail->type == 'return' ? 'selected' : '' }}>Return</option>
													</select>
												</td>
												<td><input type="text" name="product_code[]" class="form-control" value="{{ $detail->product->product_code }}" readonly></td>
												<td><input type="text" name="product_name[]" class="form-control" value="{{ $detail->product->name }}" readonly></td>
												<td><input type="number" name="quantity[]" class="form-control qty" min="1" value="{{ $detail->quantity }}"></td>
												<td><input type="number" name="price[]" class="form-control price" min="0" value="{{ $detail->price }}"></td>
												<td><input type="number" name="total[]" class="form-control total" value="{{ $detail->total }}" readonly></td>
                              ...
                              <div class="row mt-3">
                                 <div class="col-md-4">
                                    <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="comment" id="comment" aria-label="Comment" aria-describedby="basic-addon1" value="{{ $invoice->comment }}">
                                    </div>
                                 </div>
                                 @php
   ...
   <div class="col-md-12" style="background-color: #f3f7f9; padding:20px;">
      <div class="input-group">
         ...
         <input type="text" class="form-control" name="total_amount" id="total_amount" 
                ...
                aria-label="Total" aria-describedby="basic-addon1" readonly style="background-color: #ddd;">
      </div>
      <div class="input-group mt-3">
         <span class="input-group-text" id="basic-addon1">Return</span>
         <input type="text" class="form-control return" name="return_amount" id="return_amount" 
                value="{{ $invoice->return_amount ?? '' }}" 
                aria-label="Return" aria-describedby="basic-addon1" readonly style="background-color: #ddd;">
      </div>
      <div class="input-group mt-3">
         <span class="input-group-text" id="basic-addon1">Discount</span>
         <input type="text" class="form-control" name="percentage_discount" id="percentage_discount" 
                value="{{ $invoice->percentage_discount ?? '' }}" placeholder="%" aria-describedby="basic-addon1">
         <input type="text" class="form-control" name="flat_discount" id="flat_discount" 
                value="{{ $invoice->flat_discount ?? '' }}" placeholder="Flat Discount">
      </div>
      <div class="input-group mt-3">
      ...
      <div class="input-group mt-3">
         ...
         <input type="text" class="form-control" name="payable_amount" id="payable_amount" 
                value="{{ $invoice->payable_amount ?? '' }}" aria-describedby="basic-addon1">
      </div>
      <div class="input-group mt-3">
         ...
         <input type="text" class="form-control" name="paid_amount" id="paid_amount" 
                value="{{ $invoice->payments->sum('paid_amount') ?? '' }}" aria-describedby="basic-addon1">
      </div>
      <div class="input-group mt-3">
         ...
         <input type="text" class="form-control" name="due_amount" id="due_amount" 
                value="{{ $invoice->payments->sum('due_amount') ?? '' }}" aria-describedby="basic-addon1" readonly style="background-color: #ddd;">
      </div>
      <div class="input-group mt-3">
		...
		@foreach ($invoice->payment_details as $paymentDetail)
		   <div class="input-group mt-3">
			  ...
			  <select class="form-select" name="transaction_type[]">
				 <option value="Cash" {{ $paymentDetail->transaction_type == 'Cash' ? 'selected' : '' }}>Cash</option>
				 <option value="Bkash" {{ $paymentDetail->transaction_type == 'Bkash' ? 'selected' : '' }}>Bkash</option>
				 <option value="Cash Apps" {{ $paymentDetail->transaction_type == 'Cash Apps' ? 'selected' : '' }}>Cash Apps</option>
				 <option value="DBBL" {{ $paymentDetail->transaction_type == 'DBBL' ? 'selected' : '' }}>DBBL</option>
				 <option value="Nagad" {{ $paymentDetail->transaction_type == 'Nagad' ? 'selected' : '' }}>Nagad</option>
				 <option value="Card" {{ $paymentDetail->transaction_type == 'Card' ? 'selected' : '' }}>Card</option>
				 <option value="Bank Transfer" {{ $paymentDetail->transaction_type == 'Bank Transfer' ? 'selected' : '' }}>Bank Transfer</option>
			  </select>
		   </div>
		@endforeach
      ...
      <div class="mt-3">
         <div class="d-flex justify-content-end gap-2">
            <button type="submit" class="btn btn-info" id="storeButton"> Update Invoice</button>
